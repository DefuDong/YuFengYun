...
-(BOOL)isIPhone5Screen
{
    ...
    NSLog(@"x: %f, y:%f, wide:%f,height:%f",rect.origin.x, rect.origin.y , rect.size.width, rect.size.height);
...
- (void)viewDidLoad
{
    ...
    BOOL bRet = [self isIPhone5Screen];
    if (bRet) {
...
//        NSString *strNote = [NSString stringWithString:@"网络请求失败,稍候请重试"];
        [self performSelector:@selector(showNoteView:) withObject:@"网络请求失败,稍候请重试" afterDelay:1.5];
    }
