...
@implementation CollectionArticle_Cell
...
- (void)deleteButtonPressed:(SNPopupView *)pop {
    [pop dismissModal];
    if ([self.delegate respondsToSelector:@selector(collectionArticleCellDelete:)]) {
        [self.delegate collectionArticleCellDelete:self];
    }
}
